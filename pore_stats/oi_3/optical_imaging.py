    ...
    def get_channel_enter_exit_tf(self, stage):
        ...
        * Arguments:
        ...
        if stage.get_channel_coordinates(pxs[-1], pys[-1])[0] > stage._length:
            ...
            for i in reversed(list(range(len(pxs)))):
...
def find_clusters_iterative_percentage_based(frame, template_frame, threshold_difference = .01,
    ...
    * Arguments:
    ...
    threshold_indices = list(zip(threshold_indices[0], threshold_indices[1]))
...
def find_clusters_skimage(frame, template_frame, threshold_difference = .01,
                    cluster_threshold = 20, diag = False, negative_direction = 'abs',dsize=20):
    '''
    ...
    * Arguments:
    ...
    while moves != 0:
        ...
        for i, event1 in enumerate(events):
            ...
            distance = np.array([float("inf") for m in range(len(events))], dtype=float)
            ...
            if ((i_match == i) and (distance[i_match] <= dist_threshold)):
                ...
                events = [events[k] for k in range(0, len(events)) if k != j_match]
                moves = moves + 1
...
def find_events(vid, ti = 0, tf = -1, threshold_difference = .0375, chronos = False,
 cluster_threshold = 20, template_frame = None, diag = True, blur = False, kernel = None, connect = False, connect_threshold = 0, negative_direction = 'abs', normalize = False,radius=20):
    """
    ...
    for t in range(ti, tf):
        ...
        try:
            ...
            clusters = find_clusters_skimage(
                                   ...
                                   cluster_threshold = cluster_threshold, diag = diag, negative_direction = negative_direction,dsize=radius)
