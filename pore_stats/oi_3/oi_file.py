...
class Video():
    ...
    def get_frame(self, frame, average = False, blur = False,camera = 0):
        ...
        if camera == 0:
            ...
            image_array = np.fromfile(self._file_handle,dtype = np.uint16, count = int(self._bytes_per_frame/2))
...
def cine_to_mp4(cine_file_path, mp4_file_path):
    ...
    for frame in range(cin._total_frames-1):
        try:
            print(frame)
